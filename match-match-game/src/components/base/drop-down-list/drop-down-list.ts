...
export default class DropDownList extends BaseComponent {
  ...
  constructor (classes: string[], opts: IDropDownListItem[]) {
    super('select', ['drop-down-list', ...classes]);
    this.render(opts);
  }

  private render(opts: IDropDownListItem[]): void {
    opts.forEach((obj) => {
      const opt = createElement('option', ['drop-down-list__item']) as HTMLOptionElement;
      opt.value = obj.value;
